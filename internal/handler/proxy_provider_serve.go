...
// 网址：/api/proxy-provider/{config_id}?token={user_token}
func NewProxyProviderServeHandler(repo *storage.TrafficRepository) http.Handler {
	...
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		// 从 URL 路径中提取 config_id：/api/proxy-provider/{config_id}
...
func parseVlessURI(uri string) (map[string]interface{}, error) {
	// vless://uuid@server:端口?params#name
...
// FetchAndFilterProxiesYAML 从外部订阅获取代理并应用过滤器
// 返回 YAML 字节，保留原始字段顺序并带有 2 个空格缩进
...
// 使用 util.ReorderProxyNode 对每个代理节点中的字段重新排序
...
// 将过滤器、排除过滤器、排除类型和地理 IP 过滤器应用于代理节点
func applyFiltersToNode(proxiesNode *yaml.Node, config *storage.ProxyProviderConfig) *yaml.Node {
	...
	for _, proxyNode := range proxiesNode.Content {
		...
		// 应用过滤器和 GeoIP 匹配
		// 如果设置了过滤器，则将其用作主要匹配方法
		// 如果还设置了 GeoIP 过滤器，则在 IP 匹配的情况下仍可以包含与正则表达式不匹配的节点
		if filterRegex != nil {
